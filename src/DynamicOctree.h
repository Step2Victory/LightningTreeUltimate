...
class DynamicOctree{
    ...
    struct DynamicNode {
        ...
        void create_children(){
            ...
            if(children.empty()){
                for(int i = 0; i < 2; i++){
                    for(int j = 0; j < 2; j++){
                        for(int k = 0; k < 2; k++){
                            children.push_back(new DynamicNode{.sum_q = 0,
                                             .sum_Q = 0,
        ...
        DynamicNode find_child(size_t id, const auto& charges){
            std::vector<DynamicNode*> temp(children);
