...
class Octree{
    ...
    Octree(const std::array<double, 3>& start, const std::array<double, 3>& end) {
        root = Node({(end[0] + start[0]) / 2, (end[1] + start[1]) / 2, (end[2] + start[2]) / 2},
                        std::max(std::max(end[0] - start[0], end[1] - start[1]), std::max(end[1] - start[1], end[2] - start[2])));
    }
    ...
    void add_charges(const auto& charges){
...
class Node{
    ...
    Node(const std::array<double, 3>& center, double size): center(center), size(size) {
        id_charge = NULL; 
        sum_q = 0;
        sum_Q = 0;
    }

    Node(const std::array<double, 3>& center, double size, const auto& charges): center(center), size(size) { 
        add_charges(charges);
    }
    ...
    void delete_charge(size_t id, const auto& charges){
        if(id_charge){
            ...
            (this->find_child(id, charges)).delete_charge(id, charges);
            if(!delete_children()){
    ...
    void create_children(){
        if(children.empty()){
            for(int i = -1; i < 2; i+=2){
                for(int j = -1; i < 2; i+=2){
                    for(int k = -1; i < 2; i+=2){
                        children.push_back(Node({center[0] + i * size / 2,
                                                center[1] + j * size / 2,
                                                center[2] + k * size / 2},
                                                size / 2));
                    }
